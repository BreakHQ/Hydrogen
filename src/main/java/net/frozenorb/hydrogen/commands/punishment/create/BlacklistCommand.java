...
public class BlacklistCommand {
    public static final String KICK_MESSAGE = (Object)ChatColor.RED + "Your account has been blacklisted from the " + Settings.getNetworkName() + " Network. \n\nThis type of punishment cannot be appealed.";
    ...
    private static void blacklist0(CommandSender sender, PunishmentTarget target, String reason, boolean silent, boolean clear) {
        target.resolveUUID((Callback<UUID>)((Callback)uuid -> {
            ...
            RequestResponse response = Hydrogen.getInstance().getPunishmentHandler().punish((UUID)uuid, senderUUID, Punishment.PunishmentType.BLACKLIST, reason, reason, -1L);
            ...
            if (!silent) {
                for (Player player2 : Bukkit.getOnlinePlayers()) {
                    ...
                    player2.sendMessage((Object)ChatColor.GREEN + target.getName() + " was blacklisted by " + sender.getName() + ".");
                }
            } else {
    ...
    private static FancyMessage getStaffMessage(CommandSender sender, Player bukkitTarget, String correctedTarget, String reason, boolean silent) {
        ...
        staffMessage.then("blacklisted by ").color(ChatColor.GREEN).tooltip((Object)ChatColor.YELLOW + "Reason: " + (Object)ChatColor.RED + reason).then(senderName).tooltip((Object)ChatColor.YELLOW + "Reason: " + (Object)ChatColor.RED + reason).then(".").color(ChatColor.GREEN).tooltip((Object)ChatColor.YELLOW + "Reason: " + (Object)ChatColor.RED + reason);
        return staffMessage;
    }
}
