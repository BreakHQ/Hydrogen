...
public class UnBlacklistCommand {
    ...
    public static void unblacklist(CommandSender sender, @Param(name="player") PunishmentTarget target, @Param(name="reason", wildcard=true) String reason) {
        target.resolveUUID((Callback<UUID>)((Callback)uuid -> {
            ...
            RequestResponse response = Hydrogen.getInstance().getPunishmentHandler().pardon((UUID)uuid, senderUUID, Punishment.PunishmentType.BLACKLIST, reason);
            ...
            FancyMessage staffMessage = UnBlacklistCommand.getStaffMessage(sender, bukkitTarget, target.getName(), reason);
            for (Player player : Bukkit.getOnlinePlayers()) {
                if (!player.hasPermission("minehq.punishment.create.blacklist")) continue;
